...
public class EmployeeController {
	...
	public ResponseEntity<EmployeeResponse> saveEmployee(@Valid @RequestBody Employee emp) {
		...
		empList.add(empl);
		EmployeeResponse employeeResponse=new EmployeeResponse(empList, "200", "Employee saved sucessfully", LocalDateTime.now().toString());
	...
	public ResponseEntity<EmployeeResponse> saveAllEmployee(@Valid @RequestBody List<Employee> emp) {
		...
		EmployeeResponse employeeResponse=new EmployeeResponse(empList, "200", "All employees added successfully", LocalDateTime.now().toString());
		return new ResponseEntity<EmployeeResponse>(employeeResponse, HttpStatus.OK);
	}
	...
	public ResponseEntity<EmployeeResponse> getAll() {
		...
		EmployeeResponse employeeResponse=new EmployeeResponse(empList, "200", "Success", LocalDateTime.now().toString());
		return new ResponseEntity<EmployeeResponse>(employeeResponse, HttpStatus.OK);
	}
	...
	public ResponseEntity<EmployeeResponse> updateEmployee(@Valid @RequestBody Employee emp) {
		...
		Employee empl=employeeService.saveEmployee(emp);
		empList.add(empl);
		EmployeeResponse employeeResponse=new EmployeeResponse(empList, "200", "Employee details updated", LocalDateTime.now().toString());
		return new ResponseEntity<EmployeeResponse>(employeeResponse, HttpStatus.OK);
	}
}
